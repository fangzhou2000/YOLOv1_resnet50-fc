from dataset.data import VOC2007and2012Dataset
...
class CFG:
    ...
    with_amp = True # auto mix fp32 and fp16
    ...
    transforms = Compose([
        ...
        Resize(image_size, keep_ratio=False)
    ])
...
def train():
    ...
    train_ds = VOC2007and2012Dataset(CFG.roots, CFG.class_path, CFG.transforms, 'train')
    test_ds = VOC2007and2012Dataset(CFG.roots, CFG.class_path, CFG.transforms, "val")

    train_dl = DataLoader(train_ds, batch_size=CFG.batch_size, shuffle=True,
                          num_workers=CFG.num_workers, collate_fn=collate_fn)
    test_dl = DataLoader(test_ds, batch_size=CFG.batch_size, shuffle=False,
                         num_workers=CFG.num_workers, collate_fn=collate_fn)
    yolo_net = yolo(s=CFG.S, cell_out_ch=CFG.B*5 + CFG.num_classes, backbone_name=CFG.backbone_name, pretrain=CFG.pretrain)
    yolo_net.to(device)
